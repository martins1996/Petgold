...
namespace Petgold
{
    class Petgold
    {
        ...
        SqlConnection con = new SqlConnection ("Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=C:\\Users\\Aluno\\Downloads\\Petgold-main\\Petgold\\Database1.mdf;Integrated Security=True");


        public void InserirCli(string nome, string endereco, string cidade, string celular, string email, string data_de_nascimento)
        {
            string sql = "INSERT INTO Clientes (NOME ,ENDERECO,CIDADE,CELULAR,EMAIL,DATA_DE_NASCIMENTO) VALUES ('" + nome + "','" + endereco + "','" + cidade + "','" + celular + "','" + email + "','" + data_de_nascimento + "')";
            con.Open();
        ...
        {
            string sql = "UPDATE CLIENTE  SET NOME='" + nome + "',endereco='" + endereco + "',cidade='" + cidade + "',celular='" + celular + ",email='" + email + "',data_de_nascimento='" + data_de_nascimento + "'";
            con.Open();
            SqlCommand cmd = new SqlCommand(sql, con);
            cmd.ExecuteNonQuery();
            con.Close();
